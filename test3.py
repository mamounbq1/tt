...
class CahierTextApp:
    def __init__(self, root):
        self.Cahier_texte_window = tk.Toplevel(root)
        self.Cahier_texte_window.title("Cahier de Texte")
        self.Cahier_texte_window.minsize(800, 600)
        ...
        try:
            ...
            self._create_main_layout()  # This creates main_frame
            self._create_cell_menu()    # Move this AFTER main_frame is created
    ...
    def _create_main_layout(self):
        ...
        self.main_frame = ttk.Frame(self.Cahier_texte_window, padding="10")
    ...
    def _create_regular_cell(self, row, col, slot, class_schedule):
        ...
        if class_name:
            ...
            placeholder = tk.Label(
                cell_frame,
                text="Click class name to edit",  # Changed placeholder text
                font=("Arial", 9, "italic"),
            ...
            # Bind menu to class_label instead of cell_frame
                ...
                for day_id, time_slot_id, course_id in slots:
                    ...
                    if (row, col) in self.cells and isinstance(self.cells[(row, col)], tuple):
                        ...
                        if text_widget:
                            ...
                            # Ensure class_label is clickable and bound to menu
    ...
    def _handle_control(self):
        ...
        if hasattr(self, 'current_cell'):
            ...
            if (row, col) in self.cells and isinstance(self.cells[(row, col)], tuple):
                ...
                # Label
                ttk.Label(frame, text="Select Control Type:", 
                ...
                # Radio buttons for control types
                selected_control = tk.StringVar()
                selected_control.set(controls[0])  # Default selection
                
                for control in controls:
                    ttk.Radiobutton(frame, text=control, 
                                variable=selected_control, 
                                value=control).pack(anchor='w', pady=2)
                
                # Additional notes entry
                ttk.Label(frame, text="Additional Notes:", 
                ...
                ttk.Button(button_frame, text="Cancel", 
                        command=control_window.destroy).pack(side='left')

    def _handle_custom_content(self):
        """Handle Custom Content menu option"""
        if hasattr(self, 'current_cell'):
            row, col = self.current_cell
            if (row, col) in self.cells and isinstance(self.cells[(row, col)], tuple):
                text_widget, placeholder, _ = self.cells[(row, col)]
            ...
            for (row, col), cell in self.cells.items():
                if isinstance(cell, tuple):
                    ...
                    if text_widget and text_widget.winfo_ismapped():
                        ...
                        if content:
                            # Always save the content in schedule_data
                            self.db.cursor.execute("""
                            ...
                            # Only update course_progress if content matches ma_table
                            self.db.cursor.execute("""
                                SELECT id FROM ma_table WHERE valeur = ?
                            """, (content,))
                            result = self.db.cursor.fetchone()
                            
                            if result:  # Only track progress if content is from ma_table
                                cours_id = result[0]
                                ...
                                if class_name:
                                    ...
                                    if result:
                                        class_id = result[0]
                                        course_progress[class_id] = max(cours_id, 
                                            course_progress.get(class_id, 0))
            
            # Update class_course_progress for tracked classes
    ...
    def _create_cell_menu(self):
        """Create the right-click menu for grid cells"""
    ...
    def _show_cell_menu(self, event, row, col):
        ...
        if (row, col) in self.cells and isinstance(self.cells[(row, col)], tuple):
            ...
            # Enable/disable switch option based on if there are other appearances
            ...
            self.cell_menu.entryconfig("Switch Content With...", 
                                    state='normal' if other_appearances else 'disabled')
            self.cell_menu.post(event.x_root, event.y_root)

    def _find_other_appearances(self, current_row, current_col, class_name):
        """Find other appearances of the same class in the grid"""
        appearances = []
        for (row, col), cell in self.cells.items():
    ...
    def _handle_switch_content(self):
        """Handle switching content between class appearances"""
        if hasattr(self, 'current_cell'):
    ...
    def _show_switch_dialog(self, other_appearances, class_name):
        """Show dialog for selecting which appearance to switch with"""
        switch_window = tk.Toplevel(self.main_frame)
        switch_window.title(f"Switch Content - {class_name}")
        switch_window.geometry("400x300")
        
        # Create a frame to hold the appearances list
        frame = ttk.Frame(switch_window, padding="10")
        frame.pack(fill='both', expand=True)
        
        ttk.Label(frame, text=f"Select appearance to switch content with:", 
                font=("Arial", 11, "bold")).pack(pady=(0, 10))
        
        # Create a listbox to show other appearances with their content
        listbox = tk.Listbox(frame, width=50, height=10)
        listbox.pack(fill='both', expand=True)
        
        # Populate listbox with other appearances and their content
        for row, col in other_appearances:
            ...
            day = self.columns[col - 1]  # Convert column index to day name
        ...
        def switch_content():
            ...
            if selection:
                # Get selected appearance coordinates
                target_row, target_col = other_appearances[selection[0]]
                
                # Get current cell content
                current_text_widget, _, _ = self.cells[self.current_cell]
                current_content = current_text_widget.get('1.0', tk.END).strip() \
                                if current_text_widget.winfo_ismapped() else ""
                
                # Get target cell content
                target_text_widget, target_placeholder, _ = self.cells[(target_row, target_col)]
                target_content = target_text_widget.get('1.0', tk.END).strip() \
                            if target_text_widget.winfo_ismapped() else ""
    ...
    def _update_cell_content(self, row, col, content):
        """Update cell content and handle visibility"""
        if (row, col) in self.cells and isinstance(self.cells[(row, col)], tuple):
            text_widget, placeholder, _ = self.cells[(row, col)]
            if text_widget:
                text_widget.delete('1.0', tk.END)
    ...
    def _get_time_slot_text(self, row):
        """Convert grid row to time slot text"""
...
if __name__ == "__main__":
    try:
        Cahier_texte_window = tk.Tk()
        app = CahierTextApp(Cahier_texte_window)
        Cahier_texte_window.mainloop()
    except Exception as e:
